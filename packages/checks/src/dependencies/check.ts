...
async function parseNpmAudit(
  ...
  for (const [packageName, vulnerability] of Object.entries(
    ...
    findings.push({
      id: `dependency-${packageName}-${advisory?.title ?? 'vulnerability'}`,
      severity,
...
function parsePnpmAudit(output: string): Finding[] {
  ...
  return Object.entries(audit.advisories ?? {}).map(
    ([advisoryId, advisory]) => ({
      id: `dependency-${advisoryId}`,
  ...
  async run(context: ScanContext): Promise<Finding[]> {
    switch (context.project.packageManager) {
      case 'npm': {
        ...
        return output
        ? await parseNpmAudit(output, context.rootDir)
        : [];
      }
