...
WreckCheck needs Node.js 20 or newer. Nothing is uploaded anywhere — every check
runs locally against the working tree.
...
| Package | Role |
| --- | --- |
| `packages/core` | Project discovery, config and policy, scoring, the scan runner, and the verification adapters |
| `packages/checks` | The checks themselves, and the Docker rules they are built from |
| `packages/reporter` | Terminal, JSON, SARIF, and GitHub summary output |
| `packages/cli` | The `wreckcheck` command |
| `packages/action` | The GitHub Action |
| `packages/docs` | TypeDoc setup for the API documentation |

Only `wreckcheck` is published; the rest are internal and bundled into it.
...
pnpm test:integration   # builds the CLI, then runs it end to end
pnpm lint               # Biome
pnpm format             # Biome, writing fixes
pnpm docs:build         # API documentation into docs/
```

`pnpm docs:build` builds the packages first, then runs TypeDoc over every
package's source and writes HTML to `docs/`.

TypeDoc does not support TypeScript 7 yet, so `packages/docs` pins its own
TypeScript 5 rather than holding the rest of the workspace back. Nothing else
depends on it.
